define([
  ...
  , Component
     ...
     */
    function UFOImporter(log, ufoReader, masterName, options) {
        this._log = log;
        ...
        this._layerName = options.layerName || undefined;

        if(options.classes)
            this._glyphClasses = this._reverseGlyphGroups(options.classes);

        this._addSourceGlyphClasses = 'addSourceGlyphClasses' in options
                ? !!options.addSourceGlyphClasses
                : true
                ;

        this._indicatorToContourImportType = options.contourImportTypeIndicators
                 || {
                          'C:': 'Contour'
                        , 'P:': 'PenStroke'
                    };

        this._defaultContourImportType = options.defaultContourContourImportType
                || 'penstroke';

        this._contourImportTypeFallbacks = options.contourImportTypeFallbacks
                || { 'PenStroke': 'Contour' };

        this._skipOpenContours = 'skipOpenContours' in options
                ? !!options.skipOpenContours
                : true
                ;
    }
        ...
        options = {
            readErrorCallback: function( masterName, metadata ) {
                this._log.warning('ImportController: Got an error loading glyph "'
                                + metadata.glyphName + ' for master "' + masterName
    ...
    _p._makeCPSContourPoinData = function (point, index, length) {
        //jshint unused:vars
        var data={};
        data.on = point.on;
    ...
    _p._getContourImportType = function(id) {
        ...
        if(indicatorIndex !== -1) {
            indicator = id.slice(indicatorIndex);
            type = this._indicatorToContourImportType[indicator];
        }
        return indicator || this._defaultContourImportType;
    };

    _p._importContour = function(contourImportType, item, id, i, glyphName) {
        var data, node;

        this._log.debug('Glyph "'+glyphName+'" importing contour ' + i
                                            + ' as MOM ' + contourImportType + '.');
        switch (contourImportType ) {
            case 'Contour':
                if(!item.closed && this._skipOpenContours) {
                    this._log.debug('    Glyph "'+glyphName+'" skipping contour '
                                            + i +' because it is open.');
                    return false;
                }
            ...
            case 'PenStroke':
                ...
                break;
            default:
                throw new ImportError('Import type "' + contourImportType + '" is unknown.');
        }
