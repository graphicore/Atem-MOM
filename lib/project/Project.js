define([
  ...
  , UFOReader
    ...
    function MetapolatorProject(io, baseDir, fsEvents, cpsLibIo) {
        ...
        this._log = new Logger().setLevel(Level.INFO);
    ...
    Object.defineProperty(_p, 'baseMasterCPSFile', {value: 'MOM/primal.cps'});
    ...
    _p._initLogging = function(logRecords) {
        ...
        var fh = new CallbackHandler(this._io.appendFile.bind(this._io, true, this.logFile));
    ...
    _p._loadProject = function(async) {
        var data = [
                this._persistence.readProject(true)
              , this._persistence.readMOMData(true, 'multivers')
            ]
    ...
    */
    _p.createMaster = function (async, sourceMasterName, masterName, cpsString) {
        FIXME;
            // id, classes, properties, attachments
        ...
        var cloneArgs = [true, true, false, false];
    ...
    _p.cloneMaster = obtain.factory(
        {
            cloneArgs: [function() {
                FIXME;
                // this clone contains all: attachments, ids, classes, properties
                return [true, false];
            }]
          , sourceMOM: [false, 'sourceMasterName', _p._openMaster]
