  ...
  , UFOReader
    ...
    */
    _p.createMaster = function (async, sourceMasterName, masterName, cpsString) {
        ...
        // FIXME: Why Properties? I suppose some are good and some are not!
        // Master/Glyph properties good, below bad, even harmful!
        // We should start earlier with a notion of a "baseNode", then we
        // can use that to provide fallback data.
        // Properties and Attachments on Glyph-Level need a good approach
        // here! This will become interesting!
        var cloneArgs = [1 | 2];
    ...
    _p.exportUFOInstance = function(async, masterName, targetFileName, precision) {
        var ufoExporter = this._getUFOExporter(false, this._plainIO, masterName, targetFileName, precision);
        function onExporter(ufoExporter) {
